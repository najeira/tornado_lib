...
class SMTP:
    ...
    @gen.engine
    def _get_stream(self, host, port, timeout, callback):
        sock, sockaddr = self._get_socket(host, port, timeout)
        stream = iostream.IOStream(sock)
        yield gen.Task(stream.connect, sockaddr)
        callback(stream)

    @gen.engine
    def _get_ssl_stream(self, host, port, timeout, callback):
        sock, sockaddr = self._get_ssl_socket(host, port, timeout)
        stream = iostream.SSLIOStream(sock)
        yield gen.Task(stream.connect, sockaddr)
        callback(stream)
    ...
    def _get_ssl_socket(self, stream, do_handshake_on_connect=False, **kwds):
        sock, sockaddr = self._get_socket(host, port, timeout)
        kwds['do_handshake_on_connect'] = False
        sock = ssl.wrap_socket(sock, **kwds)
        return sock, sockaddr
    ...
    def help(self, args='', callback=None):
        ...
        yield gen.Task(self.putcmd, "help", args)
        (code, msg) = yield gen.Task(self.getreply)
        if callback:
            callback(msg)
    ...
    def noop(self, callback):
        ...
        self.docmd("noop", callback=callback)
    ...
    def expn(self, address, callback):
        ...
        yield gen.Task(self.putcmd, "expn", quoteaddr(address))
        self.getreply(callback)
    ...
    def starttls(self, keyfile=None, certfile=None, callback=None):
        ...
        (resp, reply) = yield gen.Task(self.docmd, "STARTTLS")
        if resp == 220:
            if not _have_ssl:
    ...
    def sendmail(self, from_addr, to_addrs, msg, mail_options=[],
        ...
        (code, resp) = yield gen.Task(self.mail, from_addr, esmtp_opts)
        if code != 250:
        ...
        (code, resp) = yield gen.Task(self.data, msg)
        if code != 250:
            yield gen.Task(self.rset)
